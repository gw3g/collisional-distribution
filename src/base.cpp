...
double R_hard(double eps, double T, double mu, double nf) {
  ...
  if (eps<1e-3) { // use expansion if eps<<max(pi.T,mu) ... (TODO: check!)
    return 1. - 3.*T*eps/(2.*sqr(M_PI)*mD2) - (nf-3.)*sqr(eps)/(12.*sqr(M_PI)*mD2);
  }

  double Phi_f, Phi_b;

  Phi_b = eps*sqr(T)*( sqr(M_PI)/6.   - l2b(eps/T) )
        + 2.*cube(T)*( gsl_sf_zeta(3.)- l3b(eps/T) );

  Phi_f = eps*sqr(T)*( l2f((eps+mu)/T)-l2f(+mu/T) )
        + eps*sqr(T)*( l2f((eps-mu)/T)-l2f(-mu/T) )
        + 2.*cube(T)*( l3f((eps+mu)/T)-l3f(+mu/T) )
        + 2.*cube(T)*( l3f((eps-mu)/T)-l3f(-mu/T) );

  double res = ( 6.*Phi_b + nf*Phi_f );
  res /= cube(eps)*sqr(2.*M_PI)*mD2;
  res *= 2*sqr(eps);
  return res;
}

//
// single scattering spectrum (hard & soft interpolation)
//
...
ifstream fin; // for interpolations:
...
namespace itp {
  ...
  void init(double _T, double _mu, double _nf) {
    ...
    cout << "\n--> preparing interpolation table..." << endl;
